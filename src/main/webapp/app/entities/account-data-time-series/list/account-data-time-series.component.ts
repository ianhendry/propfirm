...
export class AccountDataTimeSeriesComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.accountDataTimeSeriesService
      .query({
        page: pageToLoad - 1,
  ...
  protected onSuccess(data: IAccountDataTimeSeries[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/account-data-time-series'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
    ...
    this.accountDataTimeSeries = data ?? [];
