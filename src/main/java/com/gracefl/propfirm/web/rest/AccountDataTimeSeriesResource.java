...
import com.gracefl.propfirm.service.AccountDataTimeSeriesQueryService;
...
public class AccountDataTimeSeriesResource {
    ...
    private final AccountDataTimeSeriesQueryService accountDataTimeSeriesQueryService;
    ...
    public AccountDataTimeSeriesResource(
        ...
        AccountDataTimeSeriesQueryService accountDataTimeSeriesQueryService
    ) {
        ...
        this.accountDataTimeSeriesQueryService = accountDataTimeSeriesQueryService;
    }
    ...
    public ResponseEntity<List<AccountDataTimeSeries>> getAllAccountDataTimeSeries(
        AccountDataTimeSeriesCriteria criteria,
        Pageable pageable
    ) {
        log.debug("REST request to get AccountDataTimeSeries by criteria: {}", criteria);
        Page<AccountDataTimeSeries> page = accountDataTimeSeriesQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /account-data-time-series/count} : count all the accountDataTimeSeries.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/account-data-time-series/count")
    public ResponseEntity<Long> countAccountDataTimeSeries(AccountDataTimeSeriesCriteria criteria) {
        log.debug("REST request to count AccountDataTimeSeries by criteria: {}", criteria);
        return ResponseEntity.ok().body(accountDataTimeSeriesQueryService.countByCriteria(criteria));
    }
